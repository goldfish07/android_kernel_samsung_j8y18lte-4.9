				...
				struct device_node *np, sec_charger_platform_data_t *pdata)
{
	...
	if (IS_ERR_VALUE((__u64)pdata->chg_gpio_en)) {
		pr_info("can't parsing dt:battery,chg_gpio_en\n");
		return -ENOENT;
	}
	pr_info("battery charge enable pin = %d\n", pdata->chg_gpio_en);

	ret = (__u64) of_property_read_u32(np, "battery,chg_float_voltage", &pdata->chg_float_voltage);
	if (IS_ERR_VALUE((__u64)ret)) {
...
static int sm5708_charger_probe(struct platform_device *pdev)
{
	...
	ret = (__u64) power_supply_register(&pdev->dev, charger->psy_chg.desc, charger->psy_chg.config);
	if (ret) {
	...
	ret = (__u64) power_supply_register(&pdev->dev, charger->psy_otg.desc, charger->psy_otg.config);
	if (ret) {
