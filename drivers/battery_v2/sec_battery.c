...
static int sec_battery_probe(struct platform_device *pdev)
{
	...
	ret = (__u64) power_supply_register(&pdev->dev, battery->psy_pogo.desc, battery->psy_pogo.config);
	if (ret) {
		dev_err(battery->dev,
	...
	ret = (__u64) power_supply_register(&pdev->dev, battery->psy_ps.desc, battery->psy_ps.config);
	if (ret) {
		dev_err(battery->dev,
	...
	ret = (__u64) power_supply_register(&pdev->dev, battery->psy_wireless.desc, battery->psy_wireless.config );
	if (ret) {
		dev_err(battery->dev,
	...
	ret = (__u64) power_supply_register(&pdev->dev, battery->psy_usb.desc, battery->psy_usb.config);
	if (ret) {
		dev_err(battery->dev,
	...
	ret = (__u64) power_supply_register(&pdev->dev, battery->psy_ac.desc, battery->psy_ac.config);
	if (ret) {
		dev_err(battery->dev,
	...
	ret = (__u64) power_supply_register(&pdev->dev, battery->psy_bat.desc, battery->psy_ac.config);
	if (ret) {
		dev_err(battery->dev,
